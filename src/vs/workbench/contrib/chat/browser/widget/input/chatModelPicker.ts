...
export function buildModelPickerItems(
	...
	if (setupRequired) {
		...
		// NOVEL-BUILDER: BYOK never reports SetupRequired once a model exists
		// (isSetupRequired: "BYOK and anonymous access never report this state"),
		// but a signed-out author with none configured yet had no entry to add
		// their first key — the sign-in prompt was the only action. Surface
		// "Manage Models..." here so bring-your-own-key is reachable before, or
		// instead of, signing in to the hosted service.
	...
	if (useGroupedModelPicker) {
		...
		if (models.length) {
			...
			const markPlaced = (identifierOrId: string, metadataId?: string) => {
				...
				if (metadataId) {
					placed.add(vendorBaseKey(identifierOrId, metadataId));
				}
			};
			...
			for (const model of models) {
				if (placed.has(model.identifier) || placed.has(vendorBaseKey(model.identifier, model.metadata.id))) {
					continue;
				}
			...
			otherModels = models.filter(m => !placed.has(m.identifier) && !placed.has(vendorBaseKey(m.identifier, m.metadata.id)));
