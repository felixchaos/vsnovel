...
export class AnthropicLMProvider extends AbstractLanguageModelChatProvider {
	...
	/* NOVEL-BUILDER: pin the native Anthropic BYOK endpoint to real Anthropic. Without an explicit baseURL the SDK falls back to process.env.ANTHROPIC_BASE_URL, so a user who exports that to a relay (common for the claude CLI) would have their real sk-ant key silently sent to the relay and rejected. Users who actually want a relay should use the "Custom Endpoint" provider. */
	private static readonly API_BASE_URL = 'https://api.anthropic.com';
	...
	protected async getAllModels(silent: boolean, apiKey: string | undefined): Promise<ExtendedLanguageModelChatInformation<LanguageModelChatConfiguration>[]> {
		...
		try {
			const response = await new Anthropic({ apiKey, baseURL: AnthropicLMProvider.API_BASE_URL }).models.list();
	...
	async provideLanguageModelChatResponse(model: ExtendedLanguageModelChatInformation<LanguageModelChatConfiguration>, messages: Array<LanguageModelChatMessage | LanguageModelChatMessage2>, options: ProvideLanguageModelChatResponseOptions, progress: Progress<LanguageModelResponsePart2>, token: CancellationToken): Promise<void> {
		...
		const doRequest = async () => {
			...
			const anthropicClient = new Anthropic({ apiKey, baseURL: AnthropicLMProvider.API_BASE_URL });
