...
export class SearchSubagentPrompt extends PromptElement<SearchSubagentPromptProps> {
	async render(state: void, sizing: PromptSizing) {
		...
		// NOVEL-BUILDER: identity line below reframed from "AI coding research assistant ... across a codebase" to manuscript research (this subagent runs for every model — DeepSeek/grok/kimi included). Marker lives here as a TS comment, NOT as a {/* */} JSX child of <SystemMessage> — that becomes an invalid `undefined` message child at runtime (typecheck won't catch it).
		return (
			<>
				<SystemMessage priority={1000}>
					You are an AI research assistant that uses search tools to gather information. You can call tools to search for information and read files across a manuscript.<br />
					<br />
					{thoroughnessGuidance && (<>{thoroughnessGuidance}<br /><br /></>)}
					Once you have searched the manuscript, return a message with ONLY: the &lt;final_answer&gt; tag to provide paths and line ranges of relevant passages.<br />
					<br />
					Example:<br />
					<br />
					&lt;final_answer&gt;<br />
