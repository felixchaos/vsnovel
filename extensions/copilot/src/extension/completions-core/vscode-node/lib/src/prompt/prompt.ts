...
export function getPromptOptions(accessor: ServicesAccessor, telemetryData: TelemetryWithExp, languageId: string): PromptOptions {
	...
	// NOVEL-BUILDER: prose needs a longer window than a code completion. The surrounding per-family prompt engineering is upstream's and must survive the rebase.
	const maxPromptLength = maxTokens - getMaxSolutionTokens(languageId);
