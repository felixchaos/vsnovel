...
export class CompletionsFromNetwork {
	...
	private async _genericGetCompletionsFromNetwork<T>(
		...
		// NOVEL-BUILDER: prose continuation must not stop at a newline; see src/novel/completions/proseSampling.ts. The surrounding per-family prompt engineering is upstream's and must survive the rebase.
		const temperature = getTemperatureForSamples(this.runtimeMode, n, requestContext.languageId);
