...
import { isProseLanguage, proseMaxTokens, proseStops, proseTemperature } from '../../../../../../novel/completions/proseSampling';
...
export function getTemperatureForSamples(runtime: ICompletionsRuntimeModeService, numShots: number, languageId?: string): number {
	if (runtime.isRunningInTest()) {
		return 0.0;
	}

	if (isProseLanguage(languageId)) {
		return proseTemperature(numShots);
	}
...
// NOVEL-BUILDER: prose languages need different sampling than code.
// See src/novel/completions/proseSampling.ts for the reasoning.
...
export function getMaxSolutionTokens(languageId?: string): number {
	if (isProseLanguage(languageId)) {
		return proseMaxTokens();
	}
