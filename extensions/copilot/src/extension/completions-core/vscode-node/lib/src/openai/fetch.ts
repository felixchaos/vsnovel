...
export class LiveOpenAIFetcher extends OpenAIFetcher {
	...
	async fetchAndStreamCompletions(
		...
		const request: CompletionRequest = {
			...
			// NOVEL-BUILDER: sampling parameters come from proseSampling for prose languages. The surrounding per-family prompt engineering is upstream's and must survive the rebase.
			max_tokens: getMaxSolutionTokens(params.languageId),
			temperature: getTemperatureForSamples(this.runtimeModeService, params.count, params.languageId),
			top_p: getTopP(),
			n: params.count,
